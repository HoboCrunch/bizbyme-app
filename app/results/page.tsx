  ...
  return (
    <div className="min-h-screen">
      ...
      <header className="sticky top-0 z-50 bg-[#1a1a1a] border-b border-gray-800 backdrop-blur-sm bg-opacity-95">
        <div className="p-4 md:p-6 flex items-center justify-between gap-4">
          <Link href="/" className="flex items-center gap-2.5 hover:opacity-80 transition-opacity shrink-0">
            ...
            />
            <h1 className="text-lg font-normal hidden md:block">Biz By Me</h1>
          </Link>

          {searchParams && (
            <h2 className="text-lg md:text-2xl font-light text-center flex-1 px-4">
              The Best Upcoming Events Near <span className="font-normal">{searchParams.zipCode}</span>
            </h2>
          )}

          <Link
            href="/"
            className="flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-gray-700 hover:border-gray-600 px-4 py-2 rounded-full transition-all text-sm shrink-0"
          >
        ...
        {events.length > 0 && (
          <div className="mt-12 flex justify-center">
            <button
              ...
              className="flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-gray-700 hover:border-gray-600 px-6 py-3 rounded-full transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loadingMore ? (
                <>
                ...
                <>
                  ...
                  Search for more events near me
                </>
              )}
            </button>
          </div>
      ...
      <footer className="p-6 border-t border-gray-800 text-center text-gray-500 text-sm">
        <p>Found {events.length} event{events.length !== 1 ? 's' : ''}</p>
      </footer>
    </div>
  );
}
...
function EventCard({ event }: { event: Event }) {
  ...
  return (
    <div className="bg-[#2a2a2a] border border-gray-700 rounded-xl p-5 hover:border-gray-600 transition-all duration-200">
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          {event.tags && event.tags.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {event.tags.slice(0, 3).map((tag, index) => (
                <span
                  key={index}
                  className="px-2 py-0.5 bg-white/5 border border-gray-700 rounded-full text-xs text-gray-400"
                >
                  {tag}
                </span>
              ))}
          ...
          {link && (
            <a
              ...
              className="flex items-center gap-1.5 bg-white/5 hover:bg-white/10 border border-gray-700 hover:border-gray-600 px-3 py-1.5 rounded-full transition-all text-xs font-medium ml-auto"
            >
              Register
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
